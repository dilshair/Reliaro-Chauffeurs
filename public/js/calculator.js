...
function renderResults(data, params) {
  ...
  /* Service area banner */
  const saEl = document.getElementById('results-area-note');
  if (saEl) {
    const sa = data.serviceArea;
    if (sa && sa.message) {
      const isEnquiry = sa.status === 'enquiry';
      saEl.className = 'area-note ' + (isEnquiry ? 'area-note-enquiry' : 'area-note-warn');
      saEl.innerHTML =
        '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg><span>' + sa.message + '</span>';
      saEl.hidden = false;
    } else {
      saEl.hidden = true;
    }
  ...
  /* Stash service area status for booking URL */
  window._areaStatus = data.serviceArea ? data.serviceArea.status : 'in_area';
...
function buildBookingUrl(r, data, params) {
  ...
  const q = new URLSearchParams({
    ...
    areaStatus:   window._areaStatus || 'in_area',
  });
