...
router.post('/', async (req, res) => {
  ...
  if (areaStatus === 'enquiry')              extras.push('⚠ OUT-OF-AREA PICKUP — manual confirmation required');
  if (areaStatus === 'out_of_area')          extras.push('⚠ OUTSIDE SERVICE AREA — subject to confirmation');
  ...
  /* If Stripe payment requested — redirect to payment.
     BUT skip auto-charge for out-of-area / enquiry trips —
     we confirm availability first, then charge manually. */
  const needsConfirmation = areaStatus === 'enquiry' || areaStatus === 'out_of_area';
  if (paymentMethod === 'stripe' && parseFloat(totalPrice) > 0 && !needsConfirmation) {
